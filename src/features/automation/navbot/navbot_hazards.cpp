...
void navbot_hazards::add_soft_crumb_blacklist(nav_area_id area_id, nav_edge_id edge_id, float current_time, float duration, float cost)
{
  if (duration <= 0.0f || cost <= 0.0f || (!area_id.valid() && !hazard_nav_edge_valid(edge_id)))
  {
    return;
  }
  ...
  if (area_id.valid())
  {
    ...
    for (auto& record : records_)
    {
      ...
      record.cost = std::max(record.cost, cost);
      if (record.expire_time > 0.0f && record.expire_time < expire_time)
      {
        record.expire_time = expire_time;
        changed_record = true;
